    ...
    export class EmailTeamGrid extends GridBase<EmailTeamRow, any> {
        ...
        protected getButtons(): Serenity.ToolButton[] {
            ...
            buttons.push({
                ...
                onClick: () => {
                    ...
                    AdvanceCRM.Common.TransferProgress.download({
                        url: '/Services/EmailTeam/EmailTeam/ListExcel',
                        fields: fields,
                        title: 'Exporting to Excel',
                        preparingText: 'Building the Excel file on the server…',
                        fileName: 'EmailTeam.xlsx'
                    });
                }
            });
            ...
            buttons.push({
                ...
                onClick: () => {
                    AdvanceCRM.Common.TransferProgress.download({
                        url: '/Services/EmailTeam/EmailTeam/DownloadTemplate',
                        method: 'POST',
                        title: 'Preparing template',
                        preparingText: 'Building the template on the server…',
                        fileName: 'EmailTeam_Template.xlsx'
                    });
                }
            });
            ...
            buttons.push({
                ...
                icon: 'fa-file-import',
                onClick: () => {
                    let fileInput = document.getElementById('emailteam-excel-import-input') as HTMLInputElement;
                    if (!fileInput) {
                        fileInput = document.createElement('input');
                        fileInput.type = 'file';
                        fileInput.accept = '.xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet';
                        fileInput.style.display = 'none';
                        fileInput.id = 'emailteam-excel-import-input';
                    ...
                    fileInput.onchange = () => {
                        if (fileInput.files && fileInput.files.length > 0) {
                            ...
                            AdvanceCRM.Common.TransferProgress.upload({
                                url: '/Services/EmailTeam/EmailTeam/ImportExcel',
                                formData: formData,
