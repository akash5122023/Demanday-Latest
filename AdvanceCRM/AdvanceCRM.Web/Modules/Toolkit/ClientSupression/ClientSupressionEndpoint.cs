...
namespace AdvanceCRM.Toolkit.Endpoints
{
    ...
    public class ClientSupressionController : ServiceEndpoint
    {
        ...
        public ActionResult DownloadTemplate(IDbConnection connection, RetrieveRequest request)
        {
            ...
            // order ExcelImport reads - it used to say "Campaign Id" for column 1 instead of "Sr No".
            string[] headers = SupressionTemplateInitializer.ClientSupressionHeaders;
        ...
        public ExcelImportResponse ExcelImport(IUnitOfWork uow, ClientSupressionExcelImportRequest request)
        {
            ...
            // A row may name a different Account Number / Campaign Id than the one picked in the
            // dialog; neither has to already exist - a name that doesn't resolve is created on the
            // fly (the Campaign cascades under its resolved Master Account).
