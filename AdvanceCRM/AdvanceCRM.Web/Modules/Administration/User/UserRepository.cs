...
using System.Linq;
using AdvanceCRM.Default;
using AdvanceCRM.Common;
using Microsoft.Extensions.DependencyInjection;
using AdvanceCRM.Administration.Entities; // for GetRequiredService
...
namespace AdvanceCRM.Administration.Repositories
{
    public partial class UserRepository : BaseRepository
    {
        ...
        private class MyRetrieveHandler : RetrieveRequestHandler<MyRow>
        {
            ...
            //    var HR = Default.UserWarehouseRow.Fields;


            //    // Row.MultipleSelectionList= (List<int>)Connection.List < Default.UserWarehouseRow >().Where(x=>x.UserId==Row.UserId).Select(x=>x.WarehouseWarehouseCode);
            //    Row.MultipleSelectionList = Connection.List<UserWarehouseRow>(q => q
            //.SelectTableFields()
            //.Select(HR.WarehouseWarehouseCode)
            //.Where(HR.UserId == Row.UserId.Value));
        ...
        private class MyListHandler : ListRequestHandler<MyRow>
        {
            ...
            protected override void ApplyFilters(SqlQuery query)
            {
                ...
                if (!permissions.HasPermission(PermissionKeys.Company))
                    query.Where(fld.CompanyId == user.CompanyId);
            }
        }
    }
}
