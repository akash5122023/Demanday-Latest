...
namespace AdvanceCRM.Administration
{
    ...
    public class UserDataScript : DataScript<ScriptUserDefinition>
    {
        ...
        protected override ScriptUserDefinition GetData()
        {
            ...
            // Build the per-user permission map fresh on every request so that permission
            // grants take effect immediately for client-side authorization checks. The
            // previous outer GetLocalStoreOnly("ScriptUserPermissions:" + user.Id, ...) kept a
            // process-local cache keyed on UserPermissionRow.GenerationKey that did not reliably
            // clear after a grant, so newly-allowed permissions only reached the client after an
            // app restart / deploy. Permissions.HasPermission already reads grants in real time
            // (see PermissionService), so this stays cheap. Only the set of permission KEYS,
            // which changes solely when code is deployed, remains cached below.
