...
namespace AdvanceCRM.Demanday.Endpoints
{
    ...
    public class DemandayTeleMarketingMISController : ServiceEndpoint
    {
        ...
        [FromServices] IDemandayTeleMarketingMISListHandler handler)
        {
            ...
            var bytes = AdvanceCRM.Web.Modules.Common.AppServices.DemandayTeleMarketingMISExcelExporter.ExportToExcel(data, qaDetails);
                ...
                using (var package = new ExcelPackage(file.OpenReadStream()))
                {
                    ...
                    for (int row = 2; row <= rowCount; row++)
                    {
                        try
                        {
                            ...
                            ExcelImportHelper.ClampStringFields(demandaytelemarketingmis);
