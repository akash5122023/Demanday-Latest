        ...
        public StandardResponse MoveToTeleMarketingMIS(IUnitOfWork uow, MoveToTeleMarketingMISRequest request)
        {
            ...
            foreach (var id in request.Ids)
            {
                ...
                var demandayTeleMarketingMIS = new DemandayTeleMarketingMISRow
                {
                    ...
                    AdressLink = demandaytelemarketingquality.AddressLink,
                    EmailFormat = demandaytelemarketingquality.EmailFormat, // Note: mapped AddressLink
                ...
                using (var package = new ExcelPackage(file.OpenReadStream()))
                {
                    ...
                    for (int row = 2; row <= rowCount; row++)
                    {
                        try
                        {
                            ...
                            ExcelImportHelper.ClampStringFields(demandaytmquality);
