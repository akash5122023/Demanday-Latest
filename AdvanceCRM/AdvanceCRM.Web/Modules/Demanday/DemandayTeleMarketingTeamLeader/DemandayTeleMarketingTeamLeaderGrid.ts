...
namespace AdvanceCRM.Demanday {
	...
	export class DemandayTeleMarketingTeamLeaderGrid extends GridBase<DemandayTeleMarketingTeamLeaderRow, any> {
			...
			buttons.push({
				...
				onClick: () => {
					...
					AdvanceCRM.Common.TransferProgress.download({
						url: '/Services/Demanday/DemandayTeleMarketingTeamLeader/ListExcel',
						fields: fields,
						title: 'Exporting to Excel',
						preparingText: 'Building the Excel file on the server…',
						fileName: 'DemandayTeleMarketingTeamLeader.xlsx'
					});
				}
			});
		...
		// Opens a dialog that offers template download and file upload for importing TM Team Leader records.
				...
				fileInput.onchange = () => {
					if (fileInput.files && fileInput.files.length > 0) {
						...
						AdvanceCRM.Common.TransferProgress.upload({
							url: '/Services/Demanday/DemandayTeleMarketingTeamLeader/ImportExcel',
							formData: formData,
				...
				buttons: [
					{
						...
						click: () => {
							...
							AdvanceCRM.Common.TransferProgress.download({
								url: '~/Services/Demanday/DemandayTeleMarketingTeamLeader/DownloadTemplate',
								request: {},
								title: 'Downloading import template',
								fileName: 'DemandayTeleMarketingTeamLeaderTemplate.xlsx'
							});
						}
					},
					{
