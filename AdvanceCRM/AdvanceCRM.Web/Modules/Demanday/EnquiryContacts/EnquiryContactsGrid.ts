            ...
            buttons.push({
                ...
                onClick: () => {
                    ...
                    AdvanceCRM.Common.TransferProgress.download({
                        url: '/Services/Demanday/EnquiryContacts/ListExcel',
                        fields: fields,
                        title: 'Exporting to Excel',
                        preparingText: 'Building the Excel file on the server…',
                        fileName: 'EnquiryContacts.xlsx'
                    });
                }
            });
            buttons.push({
                ...
                icon: 'fa-file-import',
                onClick: () => {
                    let fileInput = document.getElementById('enquirycontacts-excel-import-input') as HTMLInputElement;
                    if (!fileInput) {
                        fileInput = document.createElement('input');
                        fileInput.type = 'file';
                        fileInput.accept = '.xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet';
                        fileInput.style.display = 'none';
                        fileInput.id = 'enquirycontacts-excel-import-input';
                    ...
                    fileInput.onchange = () => {
                        if (fileInput.files && fileInput.files.length > 0) {
                            ...
                            AdvanceCRM.Common.TransferProgress.upload({
                                url: '/Services/Demanday/EnquiryContacts/ImportExcel',
                                formData: formData,
