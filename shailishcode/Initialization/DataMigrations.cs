...
namespace AdvanceCRM
{
    ...
    {
        private readonly string[] databaseKeys;
            ...
            IOptions<MultiTenancyOptions> multiTenancyOptions)
        {
            ...
            foreach (var info in SqlConnections.ListConnectionStrings() ?? Array.Empty<IConnectionString>())
            {
                var key = GetConnectionStringKey(info);
            ...
            if (keys.Count == 0)
            {
                var defaultKey = multiTenancyOptions?.Value?.DefaultConnectionKey;
            ...
            databaseKeys = keys.ToArray();
        }

        /// <summary>
        /// Attempts to read a connection string 'key' from IConnectionString using common property names.
        /// Falls back to reflection so this code compiles regardless of concrete IConnectionString shape.
        /// </summary>
        private static string? GetConnectionStringKey(IConnectionString? info)
        {
            if (info == null)
                return null;

            // common property/field names that might hold the connection's key/name
            var candidateNames = new[] { "Key", "Name", "ConnectionKey", "ConnectionStringName", "KeyName" };
            ...
            foreach (var name in candidateNames)
            {
                ...
                {
                    var val = prop.GetValue(info) as string;
                ...
                {
                    var val = field.GetValue(info) as string;
                    if (!string.IsNullOrWhiteSpace(val))
                        return val;
                }
            }

            // last resort: try ToString() if it provides useful text (avoid returning type name)
            var s = info.ToString();
            if (!string.IsNullOrWhiteSpace(s) && s.IndexOf(type.FullName, StringComparison.Ordinal) < 0)
                return s;
        ...
        private void EnsureDatabase(string databaseKey)
        {
            ...
            if (cs == null)
                throw new InvalidOperationException($"Connection string for key '{databaseKey}' was not found. Check your 'Data' section in appsettings.json and ensure the connection key is configured and available to ISqlConnections implementation.");
        ...
        private void RunMigrations(string databaseKey)
        {
            ...
            // remove these lines if you want AdvanceCRM migrations to run on your DB.
            ...
            var serviceProvider = new ServiceCollection()
                ...
                {
                    options.Namespace = "AdvanceCRM.Migrations." + databaseKey + "DB";
                })
