...
namespace AdvanceCRM.Purchase {
    ...
    export class PurchaseOrderDialog extends DialogBase<PurchaseOrderRow, any> {
        ...
        getToolbarButtons() {
            ...
            if (Authorization.hasPermission("Proforma:Move to GRN")) {
                buttons.push({
                    title: 'To GRN',
                    icon: 'fa fa-share-square text-green',
                    hint: 'Move to GRN',
                    onClick: () => {
                        PurchaseOrderService.MoveToGRN({ //Disable if not applicable
                            ...
                            response => {
                                if (response.Id > 0) {
                                    ...
                                    new GrnTwoDialog().loadByIdAndOpenDialog(response.Id);
                                }
                                else
