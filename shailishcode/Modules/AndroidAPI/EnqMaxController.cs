...
{
    public class EnqMaxController : ApiController
    {
        List<EnquiryModel> Contact;
        public EnqMaxController()
        {
        ...
        public IEnumerable<EnquiryModel> Get(string id)
        {
            ...
            string str = "Select MAX(EnquiryNo) as EnqNo from Enquiry en,Users u where u.CompanyId=en.CompanyId and u.CompanyId=" + id;
            SqlDataAdapter sda = new SqlDataAdapter(str, con);
            DataSet ds = new DataSet();
            sda.Fill(ds);
            foreach (DataRow dr in ds.Tables[0].Rows)
