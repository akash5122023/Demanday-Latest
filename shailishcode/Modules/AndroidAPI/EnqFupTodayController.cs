...
{
    public class EnqFupTodayController : ApiController
    {
        List<EnquiryFollowupModel> Contact;
        public EnqFupTodayController()
        {
       ...
       // public IEnumerable<EnquiryFollowupModel> Get(string id, [FromUri] EnquiryNModel pagingparametermodel)
...

        {
            ...
            string str = "Select cn.Name,cn.Phone,enq.EnquiryN,enq.EnquiryNo,enq.StageID,enq.SourceID,sr.Source,st.Stage,en.Id,en.FollowUpDate,en.Status,en.FollowUpNote,en.EnquiryId,en.Details,us.DisplayName from EnquiryFollowUps en,Enquiry enq,Source sr,Stage st,Users us,Contacts cn where en.RepresentativeId=us.UserId and enq.ContactsId=cn.Id and en.EnquiryId=enq.Id and st.Id=enq.stageId and sr.Id=enq.SourceId and en.FollowupDate >= '" + dt + "' and en.FollowupDate <= '" + dt1 + "' and en.RepresentativeId =" + id + " ORDER BY en.Id DESC";
