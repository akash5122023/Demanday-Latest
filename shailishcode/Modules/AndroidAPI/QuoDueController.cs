...
{
    public class QuoDueController : ApiController
    {
        List<QuotationModel> Contact;
        public QuoDueController()
        {
...

        {
            ...
            string str = "Select en.Id,cn.Name,en.Date,cn.Phone,Status,sr.Source,st.Stage,us.DisplayName,en.AssignedId from Quotation en,Source sr,Stage st,Contacts cn,Users us where us.UserId=en.OwnerId and en.ContactsId=cn.Id and st.Id=en.stageId and sr.Id=en.SourceId and en.Date < '" + dt + "' ORDER BY en.Id DESC";
       ...
       // public IEnumerable<QuotationModel> Get(string id, [FromUri] EnquiryNModel pagingparametermodel)
...

        {
            ...
            string dt = DateTime.Now.ToString("yyyy-MM-dd") + " 00:00:00.000";
            string str = "Select en.Id,cn.Name,cn.Phone,en.Date,Status,sr.Source,st.Stage,us.DisplayName,en.AssignedId,en.QuotationN,en.QuotationNo from Quotation en,Source sr,Stage st,Contacts cn,Users us where us.UserId=en.OwnerId and en.ContactsId=cn.Id and st.Id=en.stageId and sr.Id=en.SourceId and en.OwnerId=" + id+ " and en.Date < '" + dt + "' ORDER BY en.Id DESC";
            SqlDataAdapter sda = new SqlDataAdapter(str, con);
            DataSet ds = new DataSet();
            sda.Fill(ds);
            int count = ds.Tables[0].Rows.Count;
