...
{
    public class CMStopenController : ApiController
    {
        List<CMSModel> Contact;
        public CMStopenController()
        {
       ...
       // public IEnumerable<CMSModel> Get( string id, [FromUri] EnquiryNModel pagingparametermodel)
             public HttpResponseMessage Get(string id, [FromUri] EnquiryNModel pagingparametermodel)
        {
            ...
            //con.Open();
            string dt = DateTime.Now.ToString("yyyy-MM-dd") + " 00:00:00.000";
            string str = "Select Id,ContactsId,Date,ProductsId,SerialNo,ComplaintId,Category,Amount,ExpectedCompletion,AssignedBy,AssignedTo,Instructions,BranchId,Status,CompletionDate,Feedback,AdditionalInfo,StageId,Priority,InvestigationBy,ActionBy,SupervisedBy,Observation,Action,Comments,CMSNo,DealerId,PurchaseDate,InvoiceNo,EmployeeId,ProjectId,CMSN,TicketNo from CMS where AssignedTo="+id+" and Status=1 and Date >= '" + dt + "' ORDER BY Id DESC";
            SqlDataAdapter sda = new SqlDataAdapter(str, con);
            DataSet ds = new DataSet();
            sda.Fill(ds);
            int count = ds.Tables[0].Rows.Count;
