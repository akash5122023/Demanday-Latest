...
{
    public class QuotationfollowupController : ApiController
    {
        List<QuotationFollowupModel> Contact;
        public QuotationfollowupController()
        {
       ...
       // public IEnumerable<QuotationFollowupModel> Get([FromUri] EnquiryNModel pagingparametermodel)
...

        {
            ...
            string str = "Select en.Id,en.FollowUpDate,Status,en.FollowUpNote,en.Details,us.DisplayName from QuotationFollowUps en,Users us where en.RepresentativeId=us.UserId";
      ...
      //  public IEnumerable<QuotationFollowupModel> Get(string id, [FromUri] EnquiryNModel pagingparametermodel)
        ...
        {
            SqlConnection con = new SqlConnection(Startup.connectionString);
            string str = "Select cn.Name,cn.Phone,enq.QuotationN,enq.QuotationNo,enq.StageID,enq.SourceID,sr.Source,st.Stage,en.Id,en.FollowUpDate,en.Status,en.FollowUpNote,en.QuotationId,en.Details,us.DisplayName from QuotationFollowUps en,Quotation enq,Source sr,Stage st,Users us,Contacts cn where en.RepresentativeId=us.UserId and enq.ContactsId=cn.Id and en.QuotationId=enq.Id and st.Id=enq.stageId and sr.Id=enq.SourceId and en.QuotationId =" + id + " ORDER BY en.Id DESC";

            //string str = "Select en.Id,en.FollowUpDate,Status,en.QuotationId,en.FollowUpNote,en.Details,us.DisplayName from QuotationFollowUps en,Users us where en.RepresentativeId=us.UserId and en.QuotationId =" + id +" ORDER BY en.Id DESC";
            SqlDataAdapter sda = new SqlDataAdapter(str, con);
            DataSet ds = new DataSet();
            sda.Fill(ds);
            int count = ds.Tables[0].Rows.Count;
