...
{
    public class CMSTCloseController : ApiController
    {
        List<CMSModel> Contact;
        public CMSTCloseController()
        {
            Contact = new List<CMSModel>();
        }
      //  public IEnumerable<CMSModel> Get([FromUri] EnquiryNModel pagingparametermodel)
...

        {
            ...
            {
                dynamic comid, comvalue, stage, priority, priorityValue, Assbyname, assbydisname, Assbyid, asstoname, asstodisname, asstoid, branch, branchid, investname, Supername, stat, cat, Statusname, CatName, conid, proname, productname, productsid, conname, conphone, conmail, conaddress, dealname, dealphone, dealmail, dealaddress, empname, empphone, empmail, empaddress, serial, amt, exdate, instn, Comdate, feedback, Addinfo, stageid, comment, investby, invest, Actionby, Action, Superby, CmsNo, Purdate, dealid, InvNo, EmpId, ProId, CMSN, TicketNo;//obj == DBNull.Value
                if (dr["ContactsId"] == DBNull.Value) conid = 0; else conid = (int)dr["ContactsId"];
                if (dr["SerialNo"] == DBNull.Value) serial = ""; else serial = (string)dr["SerialNo"];
                if (dr["Amount"] == DBNull.Value) amt = 0; else amt = (double)dr["Amount"];
                if (dr["ExpectedCompletion"] == DBNull.Value) exdate = DateTime.MinValue; else exdate = (DateTime)dr["ExpectedCompletion"];
                if (dr["Instructions"] == DBNull.Value) instn = ""; else instn = (string)dr["Instructions"];
                if (dr["CompletionDate"] == DBNull.Value) Comdate = DateTime.MinValue; else Comdate = (DateTime)dr["CompletionDate"];
                if (dr["Feedback"] == DBNull.Value) feedback = ""; else feedback = (string)dr["Feedback"];
                if (dr["AdditionalInfo"] == DBNull.Value) Addinfo = ""; else Addinfo = (string)dr["AdditionalInfo"];
                if (dr["StageId"] == DBNull.Value) stageid = 0; else stageid = (int)dr["StageId"];
                if (dr["Comments"] == DBNull.Value) comment = ""; else comment = (string)dr["Comments"];
                if (dr["Priority"] == DBNull.Value) priority = 0; else priority = (int)dr["Priority"];
                if (dr["InvestigationBy"] == DBNull.Value) investby = 0; else investby = (int)dr["InvestigationBy"];
                if (dr["ActionBy"] == DBNull.Value) Actionby = 0; else Actionby = (int)dr["ActionBy"];
                if (dr["SupervisedBy"] == DBNull.Value) Superby = 0; else Superby = (int)dr["SupervisedBy"];
                if (dr["Observation"] == DBNull.Value) invest = ""; else invest = (string)dr["Observation"];
                if (dr["Action"] == DBNull.Value) Action = ""; else Action = (string)dr["Action"];
                if (dr["CMSNo"] == DBNull.Value) CmsNo = 0; else CmsNo = (int)dr["CMSNo"];
                // if (dr["Comments"] == DBNull.Value) feedback = ""; else feedback = (string)dr["Comments"];
                if (dr["DealerId"] == DBNull.Value) dealid = 0; else dealid = (int)dr["DealerId"];
                if (dr["PurchaseDate"] == DBNull.Value) Purdate = DateTime.MinValue; else Purdate = (DateTime)dr["PurchaseDate"];
                if (dr["InvoiceNo"] == DBNull.Value) InvNo = ""; else InvNo = (string)dr["InvoiceNo"];
                if (dr["EmployeeId"] == DBNull.Value) EmpId = 0; else EmpId = (int)dr["EmployeeId"];
                if (dr["ProjectId"] == DBNull.Value) ProId = 0; else ProId = (int)dr["ProjectId"];
                if (dr["CMSN"] == DBNull.Value) CMSN = ""; else CMSN = (string)dr["CMSN"];
                if (dr["TicketNo"] == DBNull.Value) TicketNo = 0; else TicketNo = (int)dr["TicketNo"];
                if (dr["ProductsId"] == DBNull.Value) productsid = 0; else productsid = (int)dr["ProductsId"];
                if (dr["Category"] == DBNull.Value) cat = 0; else cat = (int)dr["Category"];
                if (dr["Status"] == DBNull.Value) stat = 0; else stat = (int)dr["Status"];
                if (dr["ComplaintId"] == DBNull.Value) comid = 0; else comid = (int)dr["ComplaintId"];
                if (dr["AssignedBy"] == DBNull.Value) Assbyid = 0; else Assbyid = (int)dr["AssignedBy"];
                if (dr["AssignedTo"] == DBNull.Value) asstoid = 0; else asstoid = (int)dr["AssignedTo"];
                ...
                Contact.Add(new CMSModel
                {
                    ...
                    Product = productname,
                    SerialNo = serial,
                    ComplaintId = (int)dr["ComplaintId"],
        ...
        //public IEnumerable<CMSModel> Get( string id, [FromUri] EnquiryNModel pagingparametermodel)
...

        {
            ...
            //con.Open();
            string dt = DateTime.Now.ToString("yyyy-MM-dd") + " 00:00:00.000";
            string str = "Select Id,ContactsId,Date,ProductsId,SerialNo,ComplaintId,Category,Amount,ExpectedCompletion,AssignedBy,AssignedTo,Instructions,BranchId,Status,CompletionDate,Feedback,AdditionalInfo,StageId,Priority,InvestigationBy,ActionBy,SupervisedBy,Observation,Action,Comments,CMSNo,DealerId,PurchaseDate,InvoiceNo,EmployeeId,ProjectId,CMSN,TicketNo from CMS where AssignedTo="+id+" and Status=2 and Date >= '" + dt + "' ORDER BY Id DESC";
            SqlDataAdapter sda = new SqlDataAdapter(str, con);
            DataSet ds = new DataSet();
            sda.Fill(ds);
            int count = ds.Tables[0].Rows.Count;
