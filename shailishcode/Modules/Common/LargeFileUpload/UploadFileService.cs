...
namespace AdvanceCRM.Common.LargeFileUpload
{
    public class UploadFileService
    {
        ...
        public UploadFileService(IConfiguration configuration, IWebHostEnvironment env)
        {
            _configuration = configuration;
            _env = env;

            _uploadSettings = new UploadSettings();
            _configuration.GetSection("UploadSettings").Bind(_uploadSettings);

            var path = _uploadSettings.Path;
            if (path.StartsWith("~"))
            {
                var relative = path.TrimStart('~', '/').Replace('/', Path.DirectorySeparatorChar);
                _uploadSettings.Path = Path.Combine(_env.ContentRootPath, relative);
            }
